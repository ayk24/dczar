...
class TrainingScheduler:
    def __init__(self, start_epoch: int = 0, max_epoch: int = 150, early_stopping_thres: int = 5):
...
class MyLRScheduler:
    def __init__(
        self, optimizer_states: Dict[str, float], start_lr: float, min_lr: float, ratio_reduce_lr: float = 0.5
    ) -> None:
        """
        Initializes a custom learning rate scheduler.

        Args:
            optimizer_states (dict): Optimizer states dictionary containing learning rate information.
            start_lr (float): Initial learning rate.
            min_lr (float): Minimum learning rate. Learning rate won't go below this value.
            ratio_reduce_lr (float, optional): Reduction ratio for learning rate. Defaults to 0.5.

        Raises:
            AssertionError: If `min_lr` or `ratio_reduce_lr` is 0.
        """
    ...
    def get_state(self) -> Optional[Dict[str, float]]:
        """
        Get the updated optimizer state with a reduced learning rate, if applicable.

        Returns:
            dict or None: Updated optimizer state dictionary if learning rate is reduced, else None.
        """
