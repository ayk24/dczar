...
class AttentionWeight(nn.Module):
    ...
    def forward(self, query: torch.Tensor, key: torch.Tensor, key_padding_mask: torch.ByteTensor = None) -> torch.Tensor:
        """
        Args:
        ...
        if key_padding_mask is not None:
            ...
            attn_weights[key_padding_mask] = -1e04  # -np.inf
...
class PzeroModelForPreTraining(nn.Module):
    ...
    def prediction(self, batch_instance: PzeroBatchInstance) -> torch.Tensor:
        ...
        last_hidden_state = self.layer_norm(outputs.last_hidden_state)
        ...
        selection_scores = self.selection_self_attn(
            query=masked_hidden_states, key=last_hidden_state, key_padding_mask=key_padding_mask
        )
...
class AsModel(nn.Module):
    ...
    def prediction(self, batch_instance: AsBatchInstance):
        """
        Args:
        ...
        # embedding layer
        last_hidden_state, _ = self.bert_model(input_ids=input_ids, predicate_position_ids=predicate_position_ids)
        ...
        label_scores[padding_mask] = -1e04
...
class AsPzeroModel(nn.Module):
    ...
    def prediction(self, batch_instance: AsPzeroBatchInstance):
        """
        Args:
