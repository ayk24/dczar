...
class PasGoldLabel(TypedDict):
    ...
    gold_cases List[PasGoldCase] : If there are multiple gold arguments due to co-reference relations, there are multiple gold cases in the list.
...
class AsPzeroTrainingInstance(TypedDict):
    ...
    case_type (str) : Target case type (one of 'DEP', 'INTRA', 'INTER', 'EXO1', 'EXO2', 'EXOG', 'INB', 'NULL')
    """
...
class AsPzeroBatchInstance(TypedDict):
    ...
    case_types (List[str]) : Target case types (one of 'DEP', 'INTRA', 'INTER', 'EXO1', 'EXO2', 'EXOG', 'INB', 'NULL')
    """
...
class PasEvaluationInstance(TypedDict):
    ...
    case_types (List[str]) : the list of case types (one of 'dep', 'intra', 'inter', 'exo1', 'exo2', 'exog', 'bunsetsu', 'null')
...
def create_pzero_batch_instance(
    pzero_masked_instances: List[PzeroMaskedInstance], padding_value: int = 0
) -> PzeroBatchInstance:
    """
    Args:
    ...
    input_ids = pad_sequence(
        [torch.LongTensor(inst["input_ids"]) for inst in pzero_masked_instances], batch_first=True, padding_value=padding_value
    )
    masked_ids = torch.LongTensor([inst["masked_idx"] for inst in pzero_masked_instances])
    xs_len = torch.LongTensor([len(inst["input_ids"]) for inst in pzero_masked_instances])
...
def create_as_batch_instance(training_instances: List[AsTrainingInstance], padding_value: int = 0) -> AsBatchInstance:
    """
    Args:
...
def create_as_pzero_batch_instance(
    training_instances: List[AsPzeroTrainingInstance], padding_value: int = 0
    ...
    input_ids = pad_sequence(
        [torch.LongTensor(inst["input_ids"]) for inst in training_instances], batch_first=True, padding_value=padding_value
    )
    ...
    batch = AsPzeroBatchInstance(
        ...
        case_types=[inst["case_type"] for inst in training_instances],
        eval_info=[inst["eval_info"] for inst in training_instances],
    )
