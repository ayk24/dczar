...
import yaml
from evaluating import compute_pas_f_score, compute_pzero_f_score, generate_pas_evaluation_instance
from instances import AS, AS_PZERO, CLOZE, PZERO
from iterators import PasBucketIterator, PzeroBucketIterator, create_cloze_task_dataloader
from logzero import logger
from loss import LossFunctionForAs, LossFunctionForAsPzero, LossFunctionForPzero
from models import CL_TOHOKU_BERT, AsModel, AsPzeroModel, BertForPAS, PzeroModelForPreTraining
from optimizers import get_optimizer
from schedulers import MyLRScheduler, TrainingScheduler
from tokenizer import load_tokenizer
from torch.optim import Adam
from torch.optim.lr_scheduler import CosineAnnealingLR
from tqdm import tqdm, trange
from transformers import AutoConfig, AutoModel, AutoModelForCausalLM, AutoTokenizer, get_linear_schedule_with_warmup
...
LOGZERO_LOG_FILE_TRAIN = "training.logzero.txt"
LOGZERO_LOG_FILE_EVAL = "evaluating.logzero.txt"
...
class PretrainingArgs:
    ...
    def set_additional_parameters(self) -> None:
        ...
        if (
            ...
            raise ValueError(
                f"Output directory already exists and is not empty: {self.output_dir}. "
        ...
        logzero.logfile(path.join(LOG_DIR_BASENAME, self.now_time, LOGZERO_LOG_FILE_TRAIN))
        logger.warning(
            f"device: {self.device}, " f"n_gpu: {self.n_gpu}, " f"16-bits training: {self.fp16}",
        )
...
class FinetuningArgs:
    ...
    now_time: str = None
    mode_type: str = "train"  # select 'train' or 'eval'
    ...
    def set_additional_parameters(self) -> None:
        assert path.exists(self.train_data_file), f"not found: {self.train_data_file}"
        assert path.exists(self.dev_data_file), f"not found: {self.dev_data_file}"
        assert path.exists(self.test_data_file), f"not found: {self.test_data_file}"
        os.makedirs(self.output_dir, exist_ok=True)
        ...
        if self.mode_type == "train":
            logzero.logfile(path.join(LOG_DIR_BASENAME, self.now_time, LOGZERO_LOG_FILE_TRAIN))
        elif self.mode_type == "eval":
            logzero.logfile(path.join(LOG_DIR_BASENAME, self.now_time, LOGZERO_LOG_FILE_EVAL))
        else:
            raise ValueError("unsupported value: {}".format(self.mode_type))
...
class TrainingComponents:
    ...
    def set_finetuning_components(self, args: FinetuningArgs) -> None:
        ...
        if args.logit_learning_rate:
            if args.model_type == AS:
                additional_params_dicts.append({"params": model.linear.parameters(), "lr": args.logit_learning_rate})
            elif args.model_type == AS_PZERO:
                additional_params_dicts.append(
                    {"params": model.sentence_self_attn.parameters(), "lr": args.logit_learning_rate}
                )
                additional_params_dicts.append({"params": model.cls.parameters(), "lr": args.logit_learning_rate})
                additional_params_dicts.append({"params": model.exo_linear_layer.parameters(), "lr": args.logit_learning_rate})
        if args.bert_learning_rate:
            additional_params_dicts.append({"params": model.bert_model.parameters(), "lr": args.bert_learning_rate})
        ...
        if args.learning_scheduler == "reduce_half":
            ...
            self.lr_scheduler = CosineAnnealingLR(
                self.optimizer, T_max=args.num_train_epochs * len(self.train_data_loader), eta_min=args.min_learning_rate
            )
        ...
        self.training_scheduler = TrainingScheduler(
            start_epoch=0, max_epoch=args.num_train_epochs, early_stopping_thres=args.early_stopping
        )
...
def extract_model_params(checkpoint: nn.Module, prefix: str) -> Dict[str, Any]:
    model_params = {
        param_name.replace(prefix, "", 1): params for param_name, params in checkpoint.items() if param_name.startswith(prefix)
    }
...
def sort_checkpoints(output_dir: str, checkpoint_prefix: str = CHECKPOINT_PREFIX) -> List[str]:
    ...
    ordering_and_checkpoint_path = [
        (path.getmtime(file_path), file_path) for file_path in glob(path.join(output_dir, "{}*".format(checkpoint_prefix)))
    ]
...
def rotate_checkpoints(output_dir: str, save_total_limit: int = None, checkpoint_prefix: str = CHECKPOINT_PREFIX) -> None:
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.model_type == PZERO:
                ...
                outputs = tc.model(inputs, labels=labels)
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    rotate_checkpoints(
                        args.output_dir, save_total_limit=args.save_total_limit, checkpoint_prefix=CHECKPOINT_PREFIX
                    )
...
def evaluate_finetuning(tc: TrainingComponents) -> float:
    ...
    f_score, f_score_zar = compute_pas_f_score(eval_instances)
    tc.writer.add_scalars("f_score", {dataset_name: f_score}, epoch)
    tc.writer.add_scalars("f_score_zar", {dataset_name: f_score_zar}, epoch)
