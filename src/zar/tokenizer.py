...
class JapaneseTokenizer(Tokenizer):
    ...
    def decode(
        ...
        return self.tokenizer.decode(token_ids, skip_special_tokens, clean_up_tokenization_spaces)
...
class JapaneseCharTokenizer(Tokenizer):
    ...
    def __init__(self, vocab: Dict[str, int], special_tokens: SpecialTokens, model_max_length: int = 512) -> None:
        ...
        re_special_tokens = {"*", "+", ".", "?", "{", "}", "(", ")", "[", "]", "^", "$", "-", "|", "/"}
        sp_tokens_str = "|".join(
            "".join(f"\{ch}" if ch in re_special_tokens else ch for ch in token) for token in self.special_tokens.values()
        )
