...
class LossFunctionForAs(LossFunction):
    ...
    def compute_loss(
        ...
        # golds for labels
        gold_positions = [
            gold_position_dict[case_name] for gold_position_dict in batch["gold_positions"] for case_name in TARGET_CASES
        ]
        golds = create_gold_sequences(
...
def loss_cross_entropy(model_output, golds):
    ...
    loss = loss_function(model_output.view(-1, model_output.shape[-1]), golds.view(-1))
...
def create_gold_sequences(
    batch_gold_ids: List[List[int]], seq_length: int, prob: bool = False, cuda: bool = False
) -> torch.Tensor:
