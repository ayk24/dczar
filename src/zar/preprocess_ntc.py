...
class Morph:
    ...
    def _set_morph_info(self, line: str) -> None:
        ...
        self.base_form = line[2] if line[2] != "*" else self.surface_form
        self.pos = str(line[3])
        self.prd_arg_info = line[7] if line[7] != "_" else ""
    ...
    def _find_case_info(self, case_name_ptn: str, case_type_ptn: str):
...
class Sentence:
    ...
    def set_sahen(self) -> None:
        ...
        for prd in self.predicates:
            ...
            if prd.index != 0 and prd.pos.startswith("動詞"):
                if len(previous_word.pos.split("-")) >= 2:
                    if previous_word.pos.split("-")[1] == "サ変接続":
                        prd.has_sahen = True
    ...
    for file_path in file_paths:
        ...
        for line in read_lines(file_path, print_log=False):
            ...
            # Bunsetsu begins
                ...
                bunsetsu = Bunsetsu(index=int(sent_index), head=int(head[:-1]))
